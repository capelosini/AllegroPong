...
int main(){
    ...
    ALLEGRO_FONT *startMenuFont = al_load_ttf_font("./fonts/startMenu.ttf", 50, 0);
    ALLEGRO_FONT *startMenuFontSmall = al_load_ttf_font("./fonts/startMenu.ttf", 25, 0);
    ...
    while (gameLoop){
            ...
            switch (renderScreen){
                case START_MENU:
                    if (ev.type == ALLEGRO_EVENT_MOUSE_BUTTON_DOWN){
                        // HUMAN VS BOT BUTTON
                        if (ev.mouse.y >= displayHeight/2-buttonSens && ev.mouse.y <= displayHeight/2+buttonSens){
                            bot=1;
                            renderScreen=GAME;
                        } 
                        // HUMAN VS HUMAN BUTTON
                        else if (ev.mouse.y >= displayHeight/2+70-buttonSens && ev.mouse.y <= displayHeight/2+70+buttonSens){
                            bot=0;
                            renderScreen=GAME;
                        }
                    }
        ...
        // RENDER SCREENS SWITCH
        switch (renderScreen){
        case START_MENU:
            al_clear_to_color(al_map_rgb(0, 20, 0));
            al_draw_text(startMenuFont, al_map_rgb(255, 255, 255), displayWidth/2, displayHeight/2-200, ALLEGRO_ALIGN_CENTER, "PONG by Cravo");
            al_draw_text(startMenuFontSmall, al_map_rgb(255, 255, 255), displayWidth/2, displayHeight/2, ALLEGRO_ALIGN_CENTER, "Human VS BOT");
            al_draw_text(startMenuFontSmall, al_map_rgb(255, 255, 255), displayWidth/2, displayHeight/2+70, ALLEGRO_ALIGN_CENTER, "Human VS Human");
            break;
    ...
    al_destroy_font(startMenuFont);
    al_destroy_font(startMenuFontSmall);
